...
do_open(const char *filename, int oflags)
{
        ...
        if(freefd!=EMFILE){
          dbg(DBG_PRINT,"get next empty fd\n");
      ...
      /*      2. Call fget to get a fresh file_t. */
        ...
        file_t *f=fget(-1);
        if(f!=NULL){
          dbg(DBG_PRINT,"get a fresh file\n");
    ...
    /*      4. Set file_t->f_mode to OR of FMODE_(READ|WRITE|APPEND) based on
        ...
        int lowflag=oflags & 3;
        int highflag=oflags & 0xf00;
        if (lowflag==0) f->f_mode=FMODE_READ;
        if (lowflag==1) f->f_mode=FMODE_WRITE;
        if (lowflag==2) f->f_mode=FMODE_READ|FMODE_WRITE;
        if(highflag==O_APPEND) f->f_mode=f->f_mode|FMODE_APPEND;
