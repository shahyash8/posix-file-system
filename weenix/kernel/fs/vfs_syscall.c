...
do_mknod(const char *path, int mode, unsigned devid)
{
	...
	{
		dbg(DBG_PRINT,"A Component of path was too long\n");
		return -ENAMETOOLONG;
	}
	...
	errcode=lookup(res_vnode,name,len,&res_vnode);	
	...
	if(errcode == -ENOENT)
	{
		...
		int retvar;
		retvar = res_vnode->vn_ops->mknod(res_vnode, name, len, mode, devid);
		return	retvar;
	}
	else
		return errcode;
...
do_mkdir(const char *path)
{
	...
	/* A dir component in the path does not exist so create it*/
	if(errcode == -ENOENT)
	{
